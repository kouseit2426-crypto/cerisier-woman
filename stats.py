...
// 「レフト」「ライト」は、相手が“どちら側の攻撃者から打ったか”（ネット際の打点）を表す分類。
// 相手と向き合って見ている図なので、相手のレフト（相手から見て左＝4番）は、こちらから見ると
// ネットの右側、相手のライト（2番）は左側になる（2026-09-04、こうせいさんの指摘で修正。
// 以前は着地点の左右を入れ替えていたが、実際に直すべきは「どちらから打ったか」を表す
// 矢印の起点（ネット上の位置）の方だった）。着地点そのものは読み取った位置のまま動かさない。
const OPPONENT_COURSE_ORIGIN = {
  left: { x: 140, y: 70 },
  right: { x: 60, y: 70 },
  middle: { x: 100, y: 70 },
  pipe: { x: 100, y: 70 },
...
function opponentCourseDiagramSvg(totalRow, filterCat, realKey) {
  ...
  catsToShow.forEach(cat => {
    const c = totalRow.categories[cat];
    if (!c) return;
    const origin = OPPONENT_COURSE_ORIGIN[cat] || { x: 100, y: 70 };
    originsUsed[`${origin.x},${origin.y}`] = origin;
    ...
    if (realPts && realPts.length) {
      ...
      realPts.forEach(([px, py]) => {
        arrows += `
          <line x1="${origin.x}" y1="${origin.y}" x2="${px}" y2="${py}" stroke="var(--series-blue)" stroke-width="1" stroke-dasharray="2,2" opacity="0.55" />
          <circle cx="${px}" cy="${py}" r="3" fill="var(--series-blue)" opacity="0.85" />`;
      });
      realLegendParts.push(`${CATEGORY_LABELS[cat]}（実データ${realPts.length}本）`);
      realNote = '<p class="hint" style="margin-top:6px">実データがある方向は、スクリーンショットから読み取った実際の着地位置を1本ずつ表示しています（読み取りのため近似値です）。それ以外は代表点での簡易表示です。矢印の起点は、相手のどちら側からの攻撃かを表しています。</p>';
    } else {
      const pt = OPPONENT_COURSE_POINTS[cat];
      if (!pt) return;
      const pct = totalRow.points ? Math.round((c.points / totalRow.points) * 1000) / 10 : 0;
      arrows += `
        <line x1="${origin.x}" y1="${origin.y}" x2="${pt.x}" y2="${pt.y}" stroke="var(--series-blue)" stroke-width="1.6" stroke-dasharray="3,3" />
  ...
  const legendHtml = realLegendParts.length
    ...
    : '';
  const originMarkers = Object.values(originsUsed)
    .map(o => `<circle cx="${o.x}" cy="${o.y}" r="4" fill="#e5484d" />`)
    .join('');
  return `
...
function drawMatchSummaryCanvas(canvas, data, setIndex) {
  ...
  // スタメン（開始時のローテーション）。特定のセットを見ているときはそのセット、
  // 「全セット」合計のときはスタメン情報がある最初のセットを表示する。
  // 古い試合（この機能に対応する前に取り込んだ試合）はstartingLineupが無いので、その場合は表示ごと省略する。
  const bySetArr = data.scoreProgression.bySet || [];
  const lineupSetIdx = setIndex !== null
    ? setIndex
    : bySetArr.findIndex(s => s.startingLineup && s.startingLineup.length === 6);
  const lineupSet = lineupSetIdx >= 0 ? bySetArr[lineupSetIdx] : null;
  const lineup = (lineupSet && lineupSet.startingLineup && lineupSet.startingLineup.length === 6)
    ? lineupSet.startingLineup
    : null;
  const lineupGap = 8;
  const lineupBoxW = (W - PAD * 2 - lineupGap * 2) / 3;
  const lineupBoxH = 44;
  const lineupSectionY = bodyTop + 14 + tilesH + 30;
  const lineupBlockH = lineup ? (14 + lineupBoxH * 2 + lineupGap + 30) : 0;

  const playerSectionY = lineupSectionY + lineupBlockH;
  const tableTop = playerSectionY + 26;
  ...
  // スタメン（開始時のローテーション）
  if (lineup) {
    sectionTitle(`スタメン（第${lineupSetIdx + 1}セット開始時点）`, lineupSectionY);
    const front = [lineup[3], lineup[2], lineup[1]]; // P4,P3,P2（ネット側）
    const back = [lineup[4], lineup[5], lineup[0]];  // P5,P6,P1（サーバー）
    [front, back].forEach((rowNums, rIdx) => {
      rowNums.forEach((num, cIdx) => {
        const bx = PAD + cIdx * (lineupBoxW + lineupGap);
        const by = lineupSectionY + 14 + rIdx * (lineupBoxH + lineupGap);
        ctx.fillStyle = '#f6f7fa';
        ctx.strokeStyle = '#e4e4e0';
        summaryRoundRect(ctx, bx, by, lineupBoxW, lineupBoxH, 8);
        ctx.fill(); ctx.stroke();

        const name = ROSTER[num] || `#${num}`;
        ctx.font = '700 13px "Hiragino Sans", "Noto Sans JP", system-ui, sans-serif';
        const displayName = summaryEllipsize(ctx, name, lineupBoxW - 16);
        const dw = ctx.measureText(displayName).width;
        ctx.fillStyle = '#1b2333';
        ctx.fillText(displayName, bx + lineupBoxW / 2 - dw / 2, by + lineupBoxH / 2 + 5);

        if (num === lineup[0]) {
          const bR = 9;
          const bcx = bx + lineupBoxW - bR - 4, bcy = by + bR + 4;
          ctx.fillStyle = '#e5484d';
          ctx.beginPath();
          ctx.arc(bcx, bcy, bR, 0, Math.PI * 2);
          ctx.fill();
          ctx.fillStyle = '#ffffff';
          ctx.font = '700 10px "Hiragino Sans", "Noto Sans JP", system-ui, sans-serif';
          const sw = ctx.measureText('S').width;
          ctx.fillText('S', bcx - sw / 2, bcy + 3.5);
        }
      });
    });
  }
