...
function showRotationTablesCombined(entries, dataList) {
  ...
  items.push({ label: '全体', rows: combineRotationRows(dataList.map(d => d.rotation.total.rows)) });
  items.forEach((item, idx) => {
    const chip = document.createElement('button');
    chip.className = 'player-chip';
    chip.textContent = item.label;
...
function summaryTeamTiles(data) {
  const t = data.team.total;
  const sob = data.sideOutBreak.total;
  return [
    { k: 'スパイク決定率', v: pct(t.kill_rate), sub: `${t.attempts}本中${t.points}本決定` },
    { k: 'サーブ効果率', v: pct(t.serve_efficiency), sub: `${t.serve_attempts}本中${t.serve_aces + t.serve_half_credit}本` },
    { k: 'レシーブAB率', v: pct(t.receive_ab_rate), sub: `${t.receive_attempts}本中${t.receive_a + t.receive_b}本` },
    { k: 'ブロック得点', v: `${t.block_points}本`, sub: `被ブロック${t.blocked}本` },
    { k: 'サイドアウト率', v: pct(sob.side_out_rate), sub: `レシーブ${sob.receive_total}本` },
    { k: 'ブレイク率', v: pct(sob.break_rate), sub: `サーブ${sob.serve_total}本` },
  ];
}

function summaryPlayerRows(data) {
  return (data.total || [])
...
function drawMatchSummaryCanvas(canvas, data) {
  const W = 720;
  const PAD = 26;
  ...
  const tiles = summaryTeamTiles(data);
  const players = summaryPlayerRows(data);
  ...
  setScores.forEach(s => {
    ...
    summaryRoundRect(ctx, sx, pillY + (pillH - 24) / 2, pw, 24, 6);
    ctx.fill();
  ...
  sectionTitle('チームスタッツ（全セット合計）', bodyTop);
  ...
  sectionTitle('選手別スタッツ', playerSectionY);
...
function openMatchSummaryWithData(data) {
  ...
  // 画面には<canvas>ではなく<img>として表示する。iPhoneのSafariなど、
  // canvas要素は長押しで保存できない環境でも、img要素なら長押しで保存できるため。
  const canvas = document.createElement('canvas');
  const draw = () => {
    if (summaryData !== data) return; // 描画中に別の試合が選ばれていたら古い結果は捨てる
    drawMatchSummaryCanvas(canvas, data);
    summaryImageDataUrl = canvas.toDataURL('image/png');
...
function saveMatchSummaryImage() {
  ...
  link.download = `${dateLabel}_${summaryData.opponent}_サマリー.png`;
