  ...
  <div class="page" id="page2">
    ...
    <div class="card collapsible closed" id="rotationCard">
      ...
      <div class="collapsible-body">
        ...
        <h4 style="margin:16px 0 8px;font-size:14px">相手が決めた攻撃のコース（％）</h4>
        <p class="hint" style="margin-top:-4px">
          相手が得点した攻撃だけを、コンビの分類でレフト／ミドル／ライト／バックアタックに分けています。
          実際の着地位置ではなく、コースの分類ごとの代表的な位置に矢印を表示した簡易図です。
        </p>
...
function opponentCourseDiagramSvg(totalRow, filterCat) {
...
function setOpponentCourseFilter(key) {
  ...
  renderOpponentCourseSection(CURRENT_OPP_COURSE_ROW);
}

function renderOpponentCourseSection(totalRow) {
  ...
  if (diagramEl) diagramEl.innerHTML = opponentCourseDiagramSvg(totalRow, CURRENT_OPP_COURSE_FILTER);
}

function selectRotationSet(rows, chipEl, oppRows) {
  ...
  const oppTotalRow = (oppRows && oppRows.length)
    ? (oppRows.find(r => r.rotation === '合計') || oppRows[oppRows.length - 1])
    : null;
  renderOpponentCourseSection(oppTotalRow);
}
...
function showRotationTables() {
  ...
  const items = DATA.rotation.bySet.map((s, i) => ({
    ...
    rows: s.rows,
    oppRows: (DATA.opponentRotation && DATA.opponentRotation.bySet[i]) ? DATA.opponentRotation.bySet[i].rows : null,
  }));
  items.push({
    label: '全セット合計',
    rows: DATA.rotation.total.rows,
    oppRows: DATA.opponentRotation ? DATA.opponentRotation.total.rows : null,
  });
  items.forEach((item, idx) => {
    const chip = document.createElement('button');
    chip.className = 'player-chip';
    chip.textContent = item.label;
    chip.addEventListener('click', () => selectRotationSet(item.rows, chip, item.oppRows));
    if (idx === items.length - 1) chip.classList.add('selected');
    btnEl.appendChild(chip);
  });
  const last = items[items.length - 1];
  selectRotationSet(last.rows, btnEl.children[items.length - 1], last.oppRows);
}
...
function showRotationTablesCombined(entries, dataList) {
  ...
  const items = dataList.map(d => ({
    ...
    oppRows: d.opponentRotation ? d.opponentRotation.total.rows : null,
  }));
  items.push({
    label: '全体',
    rows: combineRotationRows(dataList.map(d => d.rotation.total.rows)),
    oppRows: combineRotationRows(dataList.map(d => (d.opponentRotation && d.opponentRotation.total.rows) || [])),
  });
  items.forEach((item, idx) => {
    const chip = document.createElement('button');
    chip.className = 'player-chip';
